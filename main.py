...
PASSWORD = ""
#  实习完整区域：城市+区+街道+具体地址
InternshipLocation = "重庆市綦江区登瀛大道36号移通学院"
# 你需要签到的城市 和上方对应
City = "重庆市"
# 实习备注
Reason = "远程实习"
...
class Xyb:
    def __init__(self):
        ...
        self.ua = UserAgent(browsers=['chrome', 'firefox', 'safari'], platforms=['mobile'])
        self.Session.headers.setdefault("User-agen", self.ua.random)
        with open('xyb.js', 'r') as file:
            js_code = file.read()
        self.ctxl = execjs.compile(js_code)
        self.model = "iPhone 15<iPhone15,4>"
        self.Host = "https://xcx.xybsyw.com"

    def _setHeaders(self, url, data):
        self.Session.headers = self.ctxl.call('getHeaders', url, data)
        # print(self.ctxl.call('getHeaders', url, data))

    def _post(self, url, **kwargs):
        data = kwargs.get('data', None)
        if data is None:
            data = kwargs.get("json", {})
        self._setHeaders(url, data)
    ...
    def _random(self):
        # 生成一个不包含前导零的9位数
        nine_digit_number = random.randint(1, 9)
        for _ in range(8):
            nine_digit_number = nine_digit_number * 10 + random.randint(0, 9)
    ...
    def _getBlogs(self):
        reports = [
            ...
            [
                "经过一段时间的工作，又开始踏上了新的征程，进行新的锻炼。经过一段时间的匆匆忙碌后，终于有了可以闲下来的一点时间。可以稍微的偷懒一下，好好的享受一下工作之余的快乐。在这以后才上体验生活那种家庭生活的开始，开始我的新的工作：学习怎么做好日常生活中的细事，比如说说是挖排水沟，看似一件简单事，但是实际哂纳感做起来就显得的非常的棘手，对于我们刚步入社会的新大学生来说无疑是一个重大的考验，虽然我并不知道怎么去弄，更不知道如何弄好，但是我还是在其中努力的学习，领悟其中的心得。这些活原来在家里并要自己去做，但我在这里能够亲身的体验到家人的辛苦与劳累。只有自己亲身经历，才能深刻体会。",
            ],
            [
    ...
    def login(self, username, password):
        ...
        api = self.Host + "/login/login.action"
        req = self._post(api, data=submit)
        req_json = req.json()
        if req_json['code'] != '200':
            # print(req_json['msg'])
            return [False, req_json['msg']]
        sessionId = req_json["data"]["sessionId"]
        encryptLoginerId = req_json["data"]["encryptLoginerId"]
        phone = req_json["data"]["phone"]
        # print("登录成功")
        return [True, req_json['data']]

    def GetPlanDefault(self):
        api = self.Host + "/student/clock/GetPlan!getDefault.action"
        req = self._post(api)
        req_json = req.json()
        # print(req_json)
        if req_json['code'] != '200':
            return False
        traineeId = req_json["data"]['clockVo']["traineeId"]
        return traineeId

    def _jwd(self, internshipLocation, city):
        ...
        while True:
            try:
                ...
                x = res_json["result"]["location"]["lng"]
                y = res_json["result"]["location"]["lat"]
                print("获取经纬度坐标成功")
                break
            except:
    ...
    def gcj2bdString(self, value, mapType):
        if mapType == "BAIDU":
            ...
            return str((z * math.cos(theta) + 0.0065)) + ',' + str((z * math.sin(theta) + 0.006))
        elif mapType == "GAODE":
            return value

    def bd2gcjString(self, value, mapType):
        if mapType == "BAIDU":
            x_pi = 3.14159265358979324 * 3000.0 / 180.0
            ll = value.split(",")
            x = float(ll[0]) - 0.0065
            ...
            z = math.sqrt(x * x + y * y) - 0.00002 * math.sin(y * x_pi)
            theta = math.atan2(y, x) - 0.000003 * math.cos(x * x_pi)
            return str(z * math.cos(theta)) + ',' + str(z * math.sin(theta))
        elif mapType == "GAODE":
            return value

    def gps(self, x, y):
        api = f"https://restapi.amap.com/v3/geocode/regeo?key=c222383ff12d31b556c3ad6145bb95f4&location={x}%2C{y}&extensions=all&s=rsx&platform=WXJS&appname=c222383ff12d31b556c3ad6145bb95f4&sdkversion=1.2.0&logversion=2.0"
        req = requests.get(api, headers={
            "User-Agent": self.ua.random
        })
        req_json = req.json()
        formatted_address = req_json['regeocode']['formatted_address']
        adcode = req_json['regeocode']['addressComponent']['adcode']
        return [formatted_address, adcode]

    def signin(self):
        # 根据用户给的东西获取经纬度，然后再根据地图给定准确地址
        jwd = self._jwd(InternshipLocation, City)
        x = jwd[0]
        y = jwd[1]
        # 补充x y
        x = str(x) + str(self._random())
        y = str(y) + str(self._random())
        # 把百度地图坐标转换为高德
        jwd = self.bd2gcjString(x + "," + y, "BAIDU")
        jwd = jwd.split(",")
        x = jwd[0]
        y = jwd[1]
        print(x, y)
        ...
        adcode = gps[1]
        print(address)
        traineeId = self.GetPlanDefault()
        if traineeId is None:
            print("traineeId获取失败")
        print("traineeId获取成功：:", traineeId)
        # 签到
        submit = {
            "traineeId": traineeId,
            "adcode": adcode,
            ...
            "lng": x,
            "address": address,
            "deviceName": "iPhone 15<iPhone15,4>",
            "punchInStatus": "0",
            "clockStatus": "2",
            "imgUrl": "",
            "reason": Reason,
        ...
        api = self.Host + "/student/clock/PostNew.action"
        req = self._post(api, data=submit)
        req_json = req.json()
        print("{}：{}".format(datetime.datetime.now().date(),req_json['msg']))

    def doWeekBlogs(self):
        ...
        if traineeId is None:
            print("traineeId获取失败")
        print("traineeId获取成功：:", traineeId)
        submit = {"traineeId": traineeId}
        api = self.Host + "/student/blog/LoadBlogDate!weekYear.action"
        req = self._post(api, data=submit)
        req_json = req.json()
        months = req_json['data']
        print("需要填写周报的月份有：")
        for i in months:
            for j in i['months']:
                print("{}{}".format(i['name'], j['name']))
        api = self.Host + "/student/blog/LoadBlogDate!week.action"
        submitBlogApi = self.Host + "/student/blog/Blog!getSubmitData.action"
        saveBlogApi = self.Host + "/student/blog/Blog!save.action"
        for i in months:
            for j in i['months']:
                year = i['id']
                month = j['id']

                submit = {"traineeId": traineeId, "year": year, "month": month, "id": ""}

                req = self._post(api, data=submit)
                req_json = req.json()
                blogTasks = req_json['data']
                for blogTask in blogTasks:
                    print("正在填写：{}{} 第{}周的周报".format(i['name'], j['name'], blogTask['week']))
                    if blogTask['status'] != 2:
                        print("{}{} 第{}周的周报已填写 跳过".format(i['name'], j['name'], blogTask['week']))
                        continue
                    title = "第{}周周报".format(blogTask['week'])
                    content = self._getBlogs()
                    submit = {
                        "blogType": "1",
                        "blogTitle": title,
                        "blogBody": content,
                    ...
                    print("保存周报")
                    req = self._post(saveBlogApi, data=submit)
                    req_json = req.json()
                    if req_json['code'] != "200":
                        print("保存周报失败:{}".format(req_json['msg']))
                        continue
                    req = self._post(submitBlogApi, data={
                        "traineeId": traineeId,
                        "blogId": "1"
                    })
                    req_json = req.json()
                    ...
                    if req_json['code'] != "200":
                        print("提交周报失败:{}".format(req_json['msg']))
                        continue
                    print("提交周报成功")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='校友邦自动签到周报——AMEN——湮灭网络工作室')

    parser.add_argument('-type', type=str, default="sign", help='类型，sign签到 zb周报')
    args = parser.parse_args()
    type = args.type.lower()
    if type == "sign":
        X = Xyb()
        flag = X.login(USERNAME, PASSWORD)
        if not flag[0]:
            print(flag[1])
        X.signin()
    elif type == "zb":
